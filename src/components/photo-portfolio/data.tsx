...
const request = <T = any,>(path: string, auth = false, method = "GET", body?: Blob) =>
	fetch(BASE + path, {
		method,
...
export function useAddPhoto() {
	return useMutation(() => ({
		mutationFn: (vars: { roll: number; filename: string; content: Blob }) =>
			request<Photo>(`/admin/photo?roll=${vars.roll}&filename=${vars.filename}`, true, "POST", vars.content),
...
export function useRemoveFeaturedCat() {
	return useMutation(() => ({
		...
		onSuccess: (_data, vars, _omr, ctx) => {
			ctx.client.setQueryData(["featured-cats"], (old: FeaturedCategory[]) =>
				old.filter((c) => c.category !== vars),
			);
		},
	}));
}
