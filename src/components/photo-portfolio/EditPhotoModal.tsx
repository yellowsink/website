...
export function EditPhotoModal(props: {
	isOpen: boolean;
	onClose: () => void;
	photo: Photo;
	goNext: () => void;
	...
	return (
		<dialog ref={modalEl} onkeydown={(ev) => ev.stopImmediatePropagation()}>
			...
			<button onclick={() => (props.goPrev(), triggerReset())}>&lt;</button>
			<button onclick={() => (props.goNext(), triggerReset())}>&gt;</button>
			...
			<EditBoxRow
				...
				starting={props.photo.is_fave ? "true" : "false"}
				key="fave"
				id={props.photo.id}
			/>
